...
const isIPhone=/iPhone|iPod/i.test(ua),isIOS=isIPad||isIPhone,isAndroid=/Android/i.test(ua),isMobile=isIOS||isAndroid||matchMedia('(max-width:800px)').matches;
...
async function run(mode='fast'){
  ...
  try{
    ...
    if(isIPad)msg(`${label}: modo de compatibilidade iPad ativado…`);else msg(`${label}: preparando imagem…`);
    ...
    const profiles=[];
    if(isIPad){
      profiles.push({model:'isnet_quint8',device:'cpu'});
    }else if(isIPhone){
      profiles.push({model:'isnet_quint8',device:'cpu'});
    }else{
      const gpu=!!navigator.gpu;
      profiles.push({model:mode==='pro'?'isnet_fp16':'isnet_quint8',device:gpu?'gpu':'cpu'});
      if(mode==='pro')profiles.push({model:'isnet_quint8',device:'cpu'});
    }
    let result=null,lastError=null;
    for(let i=0;i<profiles.length;i++){
      try{if(i)msg(`${label}: tentando modo compatível…`);result=await callRemove(removeBackground,input,mode,profiles[i]);if(result)break}
      catch(e){lastError=e;console.warn('Background removal profile failed',profiles[i],e);await sleep(120)}
    }
    if(!result)throw lastError||new Error('Resultado vazio');
    msg(`${label}: refinando transparência…`);
    ...
    raw.onload=()=>{
      ...
      if(mode==='pro'){
        ...
        for(let i=3;i<a.length;i+=4){let v=a[i]/255;v=v*v*(3-2*v);if(isIPad)v=Math.min(1,Math.max(0,(v-.03)/.94));a[i]=Math.round(v*255)}
        rx.putImageData(id,0,0)
      }
      const final=new Image();final.onload=()=>finish(final,url,mode,input);final.onerror=()=>fail('O resultado não pôde ser aberto.',url);final.src=rc.toDataURL('image/png')
    };
...
function finish(out,url,mode,input){window.recorteSplit?.setAIResult?.(out);window.recorteSplit?.enableMaskCorrection?.();if(url)URL.revokeObjectURL(url);const reduced=input&&input.w<input.iw;msg(`${mode==='pro'?'IA Profissional':'IA Rápida'} concluída ✓${isIPad?' Modo iPad seguro.':''}${reduced?' Imagem otimizada durante a IA para reduzir uso de memória.':''} Revise as bordas com o pincel.`);running=false;[btn,proBtn].forEach(b=>b&&(b.disabled=false))}
