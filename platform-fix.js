...
const block=e=>{if(e.target.closest('input[type="text"],input[type="number"],textarea'))return;e.preventDefault()};
...
const VERSION='1.4.4';const BUILD='144-20260811';window.__RECORTE_BUILD__=BUILD;
const badge=document.querySelector('.version');if(badge){badge.textContent='BETA '+VERSION;badge.dataset.build=BUILD}
async function loadSequential(src){
 if(document.querySelector(`script[data-rs-extra="${src}"]`))return;
 const already=[...document.scripts].some(s=>(s.getAttribute('src')||'').split('?')[0].endsWith('/'+src)||(s.getAttribute('src')||'').split('?')[0]===src);if(already)return;
 await new Promise((resolve,reject)=>{const s=document.createElement('script');s.src=`${src}?v=${VERSION}&build=${BUILD}`;s.dataset.rsExtra=src;s.async=false;s.onload=resolve;s.onerror=()=>reject(new Error('Falha ao carregar '+src));document.body.appendChild(s)});
}
(async()=>{const modules=['upload-core-v2.js','viewport-lock.js','precision-lines.js','detector-precision.js','precision-split.js','line-lock.js','precision-touch.js','display-quality-fix.js'];for(const src of modules){try{await loadSequential(src)}catch(err){console.error('Recorte Split bootstrap:',err)}}window.dispatchEvent(new CustomEvent('recorte-bootstrap-ready',{detail:{version:VERSION,build:BUILD}}))})();
})();