...
const VERSION='1.4.5';const BUILD='145-20260811-r1';window.__RECORTE_BUILD__=BUILD;window.__RECORTE_VERSION__=VERSION;
...
async function forceUpload145(){if(window.__RECORTE_UPLOAD_STABLE_145__)return;await new Promise((resolve,reject)=>{const s=document.createElement('script');s.src=`upload-core-v2.js?v=${VERSION}&build=${BUILD}&force=upload145`;s.dataset.rsExtra='upload-core-v2-force145';s.async=false;s.onload=resolve;s.onerror=()=>reject(new Error('Falha ao carregar motor de upload 1.4.5'));document.body.appendChild(s)})}
(async()=>{try{await forceUpload145()}catch(err){console.error('Recorte Split upload bootstrap:',err)}const modules=['viewport-lock.js','precision-lines.js','detector-precision.js','precision-split.js','line-lock.js','precision-touch.js','display-quality-fix.js','version-lock.js'];for(const src of modules){try{await loadSequential(src)}catch(err){console.error('Recorte Split bootstrap:',err)}}window.dispatchEvent(new CustomEvent('recorte-bootstrap-ready',{detail:{version:VERSION,build:BUILD}}))})();
})();